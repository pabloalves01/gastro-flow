...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://localhost:5000/usuarios');  // URL para a rota que retorna os usuários
  ...
  return (
    <div>
      ...
      {loading && <p>Carregando...</p>}  {/* Exibe "Carregando..." enquanto os dados não chegam */}

      {error && <p style={{ color: 'red' }}>{error}</p>}  {/* Exibe um erro caso haja um problema na requisição */}
